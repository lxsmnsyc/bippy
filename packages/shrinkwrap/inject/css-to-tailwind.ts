...
function convertToRGB(color: string): RGB {
  ...
  // For named colors, we could add a mapping of standard colors to RGB values
  return { r: 0, g: 0, b: 0 };
}
...
function convertToPixels(value: string): number {
  ...
  if (value.endsWith('%')) {
    return parseFloat(value) * 0.16; // Rough approximation for percentage to pixels
  }
  return parseFloat(value);
}
...
function findNearestSize(target: string, availableSizes: string[]): string {
  const targetPx = convertToPixels(target);
  const sizesPx = availableSizes.map(size => convertToPixels(size));
  const nearestPx = findNearestValue(targetPx, sizesPx);
  const index = sizesPx.indexOf(nearestPx);
  return availableSizes[index];
}

function findNearestColor(targetColor: string, availableColors: string[]): string {
  const targetRGB = convertToRGB(targetColor);
  const colorDistances = availableColors.map(color => ({
    color,
    distance: getColorDistance(targetRGB, convertToRGB(color))
  }));
  colorDistances.sort((a, b) => a.distance - b.distance);
  return colorDistances[0].color;
}

// Modify the getRemDefaultVal function to use nearest matching
function getRemDefaultVal(val: string): string | undefined {
  const defaultValues: Record<string, string> = {
    "0px": "0", "1px": "px", "0.125rem": "0.5", "0.25rem": "1", "0.375rem": "1.5",
    "0.5rem": "2", "0.625rem": "2.5", "0.75rem": "3", "0.875rem": "3.5", "1rem": "4",
    "1.25rem": "5", "1.5rem": "6", "1.75rem": "7", "2rem": "8", "2.25rem": "9",
    "2.5rem": "10", "2.75rem": "11", "3rem": "12", "3.5rem": "14", "4rem": "16",
    "5rem": "20", "6rem": "24", "7rem": "28", "8rem": "32", "9rem": "36", "10rem": "40",
    "11rem": "44", "12rem": "48", "13rem": "52", "14rem": "56", "15rem": "60",
    "16rem": "64", "18rem": "72", "20rem": "80", "24rem": "96"
  };

  const exactMatch = defaultValues[val];
  if (exactMatch) return exactMatch;

  // If no exact match, find nearest
  const nearest = findNearestSize(val, Object.keys(defaultValues));
  return defaultValues[nearest];
}

// Modify the getBorderRadiusDefaultVal function to use nearest matching
function getBorderRadiusDefaultVal(val: string): string | undefined {
  const defaultValues: Record<string, string> = {
    "0px": "-none",
    "0.125rem": "-sm",
    "0.25rem": "null",
...
// Modify the getFilterDefaultVal function to use nearest matching
function getFilterDefaultVal(val: string): string | undefined {
  const defaultValues: Record<string, string> = {
    "blur(0)": "blur-none",
    "blur(4px)": "blur-sm",
    "blur(8px)": "blur",
  ...
  const match = val.match(/^([a-z]+)\(([\d.]+)(?:px)?\)$/);
  if (!match) return undefined;

  const [_, fn, value] = match;
  const numValue = parseFloat(value);

  // Get all matching function values
  const matchingFilters = Object.entries(defaultValues)
    .filter(([k]) => k.startsWith(fn))
    .map(([k, v]) => ({
      key: k,
      value: parseFloat(k.match(/\(([\d.]+)/)?.[1] || '0'),
      class: v
    }));

  if (matchingFilters.length === 0) return undefined;

  // Find nearest value
  const nearest = matchingFilters.reduce((prev, curr) => {
